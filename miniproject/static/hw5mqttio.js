...
function startMonitoring() {
    if(!isMonitoring){
        subscribe('roomTemp');
        subscribe('roomHumidity');
        subscribe('roomLumi');
        isMonitoring = true;
    }
    publish('ISC', 'start', qos=0);
}

function stopMonitoring(){
    if(isMonitoring){
        unsubscribe('roomTemp');
        unsubscribe('roomHumidity');
        unsubscribe('roomLumi');
        isMonitoring=false;
    }
    publish('ISC', 'stop', qos=0);
}
...
function onMessageArrived(msg) { // 매개변수 msg는 도착한 MQTT 메시지를 담고 있는 객체
    ...
    else if(msg.destinationName == "roomTemp"){
        setTemp(parseFloat(msg.payloadString));
    }
    
    else if (msg.destinationName == 'roomHumidity'){
        setHumidity(parseFloat(msg.payloadString));
    }

    else if (msg.destinationName == 'roomLumi'){
        setLumi(parseFloat(msg.payloadString));
    }
